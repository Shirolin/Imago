...
| **Primary**              | **薄荷绿 (Mint)**      | **Level 2 (高)**   | 代表品牌核心（Privacy First），引导用户进入安全路径。 | 快速开始、导出图片、确认保存 |
...
#### **按钮表面处理 (Button Surface) —— 现代纯色风格：**

- **纯色表面，禁用双色渐变**：所有按钮变体使用**单一纯色填充**（`bg-[hsl(var(--cta))]` 等），**禁止使用垂直双色渐变**（如 `from-X to-Y` 的亮→暗渐变，属 Material Design 初代过时风格）。
- **层次由高光与投影承担**：纯色按钮通过**顶部 1px 白色内高光**（`inset 0 1px 0 0 rgba(255,255,255,0.2)`）+ **同色系外投影**（`btn-shadow-*` 类）营造立体感，而不是渐变。
- **Hover 点亮 (Brighten on Hover)**：CTA 悬停时底色变亮（`--cta` → `--cta-hover`），强化“启动/蓄势”的能量隐喻；Primary/Success 保持底色不变，仅投影扩散。
- **实现说明**：同色系投影由 `src/style.css` 的 `.btn-shadow-{cta,primary,success}` 类提供（亮色 `--button-shadow-opacity: 0.25`，暗色自动切换 `0.4`）。注意 `hsla()` 必须使用**斜杠 alpha 语法**（`hsla(var(--cta) / 0.25)`）——逗号语法在 CSS 变量展开后会失效，导致整个 box-shadow 声明被浏览器丢弃。
