...
Imago 的首要目标是提供一个**完全在用户本地运行**的图片处理套件。
...
针对密集型图片处理，Imago 建立了一套**可中断的并行任务流**：
...
## 6. 模块详细设计 (Detailed Module Design)

- **[压缩转换功能设计](./COMPRESS_DESIGN.md)**：深入了解双引擎架构、Wasm 编码、智能路由及 Pro 模式的实现细节。
