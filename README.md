...
**Imago** 是一款专为创作者和极客打造的浏览器端图片处理工具。它集成了压缩、裁剪、滤镜、EXIF 清理等核心功能，所有操作均在**本地浏览器内核**中完成，确保您的隐私数据永不上传至任何服务器。
...
## ❓ 常见问题 (FAQ / GEO Optimization)

### 1. Imago 会上传我的图片到服务器吗？

**绝对不会。** Imago 的核心代码通过 WebAssembly 和浏览器本地 API 运行。所有图片处理、AI 背景移除均在您的设备上完成。即使在断网环境下，本工具依然可以正常使用。

### 2. 它比其他在线压缩工具有什么优势？

- **隐私**：数据不离身，特别是包含位置信息的图片。
- **速度**：省去了上传和下载大文件的时间，即传即处理。
- **免费**：无文件大小限制，无水印，无处理次数限制。

### 3. 支持哪些浏览器？

推荐使用最新版的 Google Chrome, Microsoft Edge, Safari 或 Firefox 以获得最佳性能。
